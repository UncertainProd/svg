...
class SVGData extends Group
{
	...
	private static final colorToHex = [
		"aliceblue" => "#f0f8ff",
		"antiquewhite" => "#faebd7",
		"aqua" => "#00ffff",
		"aquamarine" => "#7fffd4",
	...
	private function applyTransform(ioMatrix:Matrix, inTrans:String):Float
	{
		...
		{
			// trace('$inTrans Matched: "${mRotationMatch.matched(1)}" | "${mRotationMatch.matched(2)}" | "${mRotationMatch.matched(3)}" | "${mRotationMatch.matched(4)}"');
			var degrees = Std.parseFloat(mRotationMatch.matched(1));

			// var rotationX = Std.parseFloat(mRotationMatch.matched(2));
			var rotationX = Std.parseFloat(mRotationMatch.matched(3));
			if (Math.isNaN(rotationX))
			{
				rotationX = 0;
			}
			// var rotationY = Std.parseFloat(mRotationMatch.matched(3));
			var rotationY = Std.parseFloat(mRotationMatch.matched(4));
